...
|----|------------------|----------------------------------------------------------|----------------------------------------------------------------------------------|
| 1  | **Generator (G)** | see `gcl-prompt-backbone.md` §1 (product overrides below) | `{{user.request}}` `{{user.operation}}` `{{user.target_resource}}` `{{user.preflight}}` `{{output.critic_feedback}}` `{{output.rubric}}` |
...
> `create-access-key` response, ALWAYS overwrite `result.ak_secret` and
> `trace[*].stdout` substring matching `secret_access_key` to `<masked>` BEFORE
