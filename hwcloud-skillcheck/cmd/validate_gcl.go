...
// runValidateGCL dispatches `hwcloud-skillcheck validate gcl-conformance` and
// `hwcloud-skillcheck validate alarm-wire-contract`.
func runValidateGCL(args []string) error {
	...
	case "-h", "--help", "help":
		fmt.Fprintln(os.Stdout, "hwcloud-skillcheck validate <gcl-conformance|alarm-wire-contract> --root <dir>")
		return nil
	default:
...
func runValidateGCLConformance(args []string) error {
	fs := newFlagSet("hwcloud-skillcheck validate gcl-conformance")
...
func runValidateAlarmWireContract(args []string) error {
	fs := newFlagSet("hwcloud-skillcheck validate alarm-wire-contract")
	root := fs.String("root", ".", "skill repository root")
	jsonOut := fs.Bool("json", false, "emit JSON report")
	if err := fs.Parse(args); err != nil {
		return err
