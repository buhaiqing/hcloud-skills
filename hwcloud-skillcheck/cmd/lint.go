...
// runLint handles `hwcloud-skillcheck lint go [--root <dir>] [--fix]`.
//
// Phase 1 replaces scripts/run_ruff.sh with a Go-native lint entry point. It
// runs `gofmt -l` (formatting) and `go vet` over the hwcloud-skillcheck module. The
...
func runLint(args []string) error {
	fs := newFlagSet("hwcloud-skillcheck lint go")
