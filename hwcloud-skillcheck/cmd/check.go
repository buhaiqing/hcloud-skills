...
import (
	...
	"github.com/buhaiqing/hcloud-skills/hwcloud-skillcheck/internal/coverage"
...
func runCheck(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("check: missing subcommand (use 'example-config'|'markdown-links'|'references-links'|'audit-results'|'lanes')")
	}
	switch args[0] {
	case "example-config":
	...
	case "-h", "--help", "help":
		fmt.Fprintln(os.Stdout, "hwcloud-skillcheck check <example-config|markdown-links|references-links|audit-results|lanes> --root <dir>")
		return nil
	default:
...
func runCheckExampleConfig(args []string) error {
	fs := newFlagSet("hwcloud-skillcheck check example-config")
...
func runCheckMarkdownLinks(args []string) error {
	fs := newFlagSet("hwcloud-skillcheck check markdown-links")
	root := fs.String("root", ".", "skill repository root")
	if err := fs.Parse(args); err != nil {
		return err
	}
...
func runCheckAdvancedCoverage(args []string) error {
	fs := newFlagSet("hwcloud-skillcheck check advanced-coverage")
	root := fs.String("root", ".", "skill repository root")
	jsonOut := fs.Bool("json", false, "emit JSON report")
...
func runCheckReferencesLinks(args []string) error {
	fs := newFlagSet("hwcloud-skillcheck check references-links")
	root := fs.String("root", ".", "skill repository root")
	jsonOut := fs.Bool("json", false, "emit JSON report")
...
func runCheckAuditResults(args []string) error {
	fs := newFlagSet("hwcloud-skillcheck check audit-results")
	root := fs.String("root", ".", "skill repository root")
	jsonOut := fs.Bool("json", false, "emit JSON report")
	if err := fs.Parse(args); err != nil {
		return err
