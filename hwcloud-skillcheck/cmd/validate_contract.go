...
// runValidateContract dispatches `hwcloud-skillcheck validate generator-contract`,
// `hwcloud-skillcheck validate safety-class`, and `hwcloud-skillcheck validate resource-scope`.
func runValidateContract(args []string) error {
	...
	case "-h", "--help", "help":
		fmt.Fprintln(os.Stdout, "hwcloud-skillcheck validate <generator-contract|safety-class|resource-scope> --root <dir>")
		return nil
	default:
...
func runValidateGeneratorContract(args []string) error {
	fs := newFlagSet("hwcloud-skillcheck validate generator-contract")
...
func runValidateSafetyClass(args []string) error {
	fs := newFlagSet("hwcloud-skillcheck validate safety-class")
	root := fs.String("root", ".", "skill repository root")
	jsonOut := fs.Bool("json", false, "emit JSON report")
	if err := fs.Parse(args); err != nil {
		return err
	...
	// skillcheckRoot = where the hwcloud-skillcheck module lives.
...
func checkSafetyClassCode(skillcheckRoot string) []string {
	...
	// skillcheckRoot is the root of the hwcloud-skillcheck module (where hwcloud-skillcheck cmd lives).
...
func runValidateResourceScope(args []string) error {
	fs := newFlagSet("hwcloud-skillcheck validate resource-scope")
	root := fs.String("root", ".", "skill repository root")
	jsonOut := fs.Bool("json", false, "emit JSON report")
	if err := fs.Parse(args); err != nil {
		return err
