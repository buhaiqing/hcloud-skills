...
that hcloud-skillcheck's core functionality works correctly using embedded fixtures
(--self-check) and deterministic inputs.

This does NOT test against the live repo — the repo may have pre-existing issues
that hcloud-skillcheck correctly detects (those are validated by `make self-check`).
...
SKILLCHECK = ROOT / "hcloud-skillcheck" / "bin" / "hcloud-skillcheck"
EMBED_FIXTURES = ROOT / "hcloud-skillcheck" / "internal" / "embed" / "fixtures"


def skillcheck_binary() -> str:
    """Resolve hcloud-skillcheck binary path, building if needed."""
    if not SKILLCHECK.exists():
        subprocess.run(
            ["go", "build", "-C", str(ROOT / "hcloud-skillcheck"), "-trimpath", "-o", "bin/hcloud-skillcheck", "."],
            check=True,
        )
...
def run_skillcheck(*args: str, binary: str) -> subprocess.CompletedProcess:
    """Run hcloud-skillcheck with the given arguments."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Smoke test: hcloud-skillcheck binary")
    parser.add_argument("--binary", default=None, help="Path to hcloud-skillcheck binary")
    ...
    print(f"Using hcloud-skillcheck binary: {binary}")
    print()
