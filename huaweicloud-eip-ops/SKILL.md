...
4. Use the Go CLI: `hwcloud-skillcheck gcl run --root .` and `hwcloud-skillcheck aggregate trace --require-traces --root .`. (The legacy Python runner was the reference spec for the wire format; the canonical Go runtime in `hwcloud-skillcheck/` is now the only path.)
