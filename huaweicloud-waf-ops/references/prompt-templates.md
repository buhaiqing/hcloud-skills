...
> persistence): see `gcl-prompt-backbone.md` §4. Product-specific note: for
> certificate upload request body, regex-replace the `private_key` /
> `BEGIN PRIVATE KEY` / `BEGIN RSA PRIVATE KEY` block to `<private_key_redacted>`
> (keep `certificate` content — public cert is not secret).
