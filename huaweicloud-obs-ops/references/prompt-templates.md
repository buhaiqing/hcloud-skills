...
> `put-bucket-policy` request body, regex-replace any `aws:SecureTransport` /
> `aws:SourceIp` values containing user identifiers to `<masked>`; mask
> `X-OBS-Signature: <sig>` too.
