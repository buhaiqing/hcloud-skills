// Package cmd: skillcheck learning subcommand.
//
//	skillcheck learning gen --root <dir>            # regenerate failure_patterns.json
//	                                                # + remediation-playbooks.json for all
//	                                                # top-frequency skills (RDS/VPC/ELB/CCE).
//	skillcheck learning trace aggregate --root <dir> --skill <skill> [--since-hours N] [--dry-run]
//	skillcheck learning trace learn    --root <dir> --skill <skill> --trace <path> [--dry-run]
//	skillcheck learning trace report   --root <dir> --skill <skill> [--json]
package cmd
...
import (
	...
	"github.com/buhaiqing/hcloud-skills/skillcheck/internal/learning"
)

func runLearning(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: skillcheck learning <gen|trace> ...")
	}
	switch args[0] {
	case "gen":
...
const learningHelp = `skillcheck learning — knowledge base + GCL trace utilities

Usage:
  skillcheck learning gen [--root <dir>]
  skillcheck learning trace <aggregate|learn|report> [--root <dir>] [--skill <id>] [--since-hours N] [--dry-run] [--json]
`

func runLearningGen(args []string) error {
	fs := newFlagSet("skillcheck learning gen")
...
func runLearningTrace(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: skillcheck learning trace <aggregate|learn|report> ...")
	}
	switch args[0] {
	case "aggregate":
...
func runTraceAggregate(args []string) error {
	fs := newFlagSet("skillcheck learning trace aggregate")
...
func runTraceLearn(args []string) error {
	fs := newFlagSet("skillcheck learning trace learn")
	root := fs.String("root", ".", "repo root")
...
func runTraceReport(args []string) error {
	fs := newFlagSet("skillcheck learning trace report")
	root := fs.String("root", ".", "repo root")
	skill := fs.String("skill", "", "skill id")
