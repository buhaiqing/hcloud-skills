...
**In-scope is mechanically decidable.** The Adjudicator's finding ledger maps finding ID → file set. It lives in the campaign record written by `hwcloud-skillcheck learning campaign record`; before that command existed, the ledger went in the campaign commit trailers:
...
Record the outcome with `hwcloud-skillcheck learning campaign record`, which stores `post_fix_rereview: reviewed | waived` in the campaign record and **rejects `waived` unless there is at least one waiver and every waiver carries all four evidence fields** (finding ID, `file:line` — JSON field `file_line` — verification command, result). Read the history back with `hwcloud-skillcheck learning campaign report`, which recomputes the campaign's process rates (dispatch failure, fix rework, post-fix re-review coverage) from raw counts and prints them beside the raw critic / adjudicator counts — so a later session inherits measurements instead of impressions, and an edited artifact cannot disagree with its own report.
...
| 2.4.0 | 2026-09-24 | `learning campaign record` ships, making §4.1 enforceable: it stores `post_fix_rereview: reviewed | waived` in `audit-results/gcl-campaigns/<id>.json` and rejects `waived` unless every waiver carries `finding_id` + `file_line` + `command` + `result`. It also writes three process rates onto the outcome row — `dispatch_failure_rate`, `fix_rework_rate` (denominator fixed to the first-round write set so the rate is not inflated by adding rounds), and `post_fix_rereview_coverage` — computed from raw counts, with zero denominators and out-of-range numerators rejected rather than clamped; rates are `*float64` so a measured `0.0` is not dropped by `omitempty` (outcome-memory spec §5.1). `learning campaign report` is the reader that makes this falsifiable: it recomputes rates from the raw counts in each artifact, prints them next to raw critic / adjudicator counts, and renders an impossible count pair as `-1` instead of hiding neighbouring campaigns. A fourth rate (`critic_false_positive_rate`) was considered and dropped: it would merge a Critic being wrong with an Adjudicator wrongly overturning a Critic, so those counts stay raw until the numerators are unambiguous. Retention is local runtime state (gitignored `audit-results/`, outcome rows pruned at 90 days) — inheritable on the same working copy, not across a fresh clone. |
