# hwcloud-skillcheck Go 迁移 — Code Review & 复盘总结
...
以下 16 个 A 类脚本已完全被 `hwcloud-skillcheck` 替代，`validate_local.py` 已改用 Go 等价性测试：
...
| `validate_local.py` | 总编排入口（已包含 hwcloud-skillcheck 等价性测试） | ~300 |
...
**建议等待**以下条件满足后清理：
- [ ] hwcloud-skillcheck 在 CI 中完全替代 `validate_local.py` 的 A 类步骤
- [ ] 等价性测试在 CI 中持续运行 > 2 周无回归
- [ ] README 中明确说明"Python 脚本已弃用，使用 hwcloud-skillcheck"
