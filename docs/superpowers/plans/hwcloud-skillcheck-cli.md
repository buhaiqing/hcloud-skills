...
- **worktree 策略**：每个 task 独立 `git worktree add ../hcloud-skills-hwcloud-skillcheck-<n> -b feature/hwcloud-skillcheck-<n>`。
...
- 实际 repo owner 未知，Plan 中用占位 `github.com/<owner>/hcloud-skills/hwcloud-skillcheck`。
...
|---|---|---|---|---|
| T1 | Go module 骨架 + embed 资源 + main 路由 | §3 | `go.mod`, `main.go`, `internal/embed/*` | `go build` 通过；`hwcloud-skillcheck --help` 输出子命令 |
...
| T11 | `lint go` 子命令 (golangci-lint 替代) | §2.4 | `cmd/lint.go` | smoke: 对 hwcloud-skillcheck 自身跑通 |
| T12 | GitHub Action 多平台编译 + Release | §3.2 | `.github/workflows/build-release.yml` | tag push 产三平台 artifact |
| T13 | 等价性对照测试套件 (Python vs Go) | §6.1 | `hwcloud-skillcheck/testdata/` + 脚本 | 全 A 类子命令退出码/失败项一致 |

## Batch → Worktree 映射

| Batch | Tasks | Branch |
|---|---|---|
| B1 (骨架) | T1, T2, T3 | `feature/hwcloud-skillcheck-b1` |
| B2 (校验器) | T4, T5, T6 | `feature/hwcloud-skillcheck-b2` |
| B3 (检查器) | T7, T8, T9 | `feature/hwcloud-skillcheck-b3` |
| B4 (编排+lint) | T10, T11 | `feature/hwcloud-skillcheck-b4` |
| B5 (分发+测试) | T12, T13 | `feature/hwcloud-skillcheck-b5` |
...
- [ ] 三平台二进制可在 Release 下载
- [ ] 干净容器（无 python3）中 `hwcloud-skillcheck validate --root <外部仓库>` 跑通
- [x] T13 等价性套件全绿
