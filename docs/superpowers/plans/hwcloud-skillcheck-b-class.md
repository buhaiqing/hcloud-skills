...
| DOC-3 | 更新 `docs/superpowers/plans/hwcloud-skillcheck-b-class.md` | 标记为 COMPLETE | commit |
...
- `README.md` — 更新 "Local validation" 章节移除 Python 引用，更新 hwcloud-skillcheck 命令列表加入新增子命令
- `README_CN.md` — 同上

### DOC-2: 用户手册 (docs/manual/)

创建/更新 `docs/manual/hwcloud-skillcheck.md`，包含：

```
# hwcloud-skillcheck 用户手册
...
# Go 版输出 (先构建)
go build -C hwcloud-skillcheck -trimpath -o bin/hwcloud-skillcheck .
./hwcloud-skillcheck/bin/hwcloud-skillcheck validate gcl-conformance --json > /tmp/go_result.json

# 对比退出码 + 关键字段
diff <(echo "exit: $?") <(echo "exit: $?")
```

结构化等价规则（同 hwcloud-skillcheck-cli.md spec）：
