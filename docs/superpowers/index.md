...
| `plans/hwcloud-skillcheck-b-class.md` | 🔶 | Spec ✅ FINAL_SPEC；DoD 存在未完成项 (Phase 4 清理) |
| `plans/hwcloud-skillcheck-cli.md` | 🔶 | 声称 COMPLETE；但 DoD 有 2 项未完成 |
...
| `specs/hwcloud-skillcheck-b-class.md` | ✅ | `plans/hwcloud-skillcheck-b-class.md` | FINAL_SPEC，3 轮 self-critique 完成 |
| `specs/hwcloud-skillcheck-cli.md` | ✅ | `plans/hwcloud-skillcheck-cli.md` | FINAL_SPEC；但 DoD 有 2 项未解决 (见下) |

---

## 未完成项详情

### 🔶 hwcloud-skillcheck-cli.md — DoD 未完成

| # | DoD 项 | 状态 | 说明 |
|---|--------|------|------|
| 1 | 三平台二进制可在 Release 下载 | ❌ | B1–B5 合并完成，但未验证 Release 发布 |
| 2 | 干净容器（无 python3）中 `hwcloud-skillcheck validate --root <外部仓库>` 跑通 | ❌ | 未在干净容器环境中验证 |

**影响**: `plans/hwcloud-skillcheck-cli.md` 标记为 COMPLETE 存在虚报。需在真实 Release 后重新验证。

### 🔶 hwcloud-skillcheck-b-class.md — DoD 未完成
...
| 7 | `docs/manual/hwcloud-skillcheck.md` 用户手册 | ❌ | Phase 4（DOC-1）待执行 |
...
1. **继续**: `hwcloud-skillcheck-b-class.md` — Phase 1 完成！下一个任务：Phase 2 (L2-A~D: 4 路并行校验命令)
...
5. **验证**: `hwcloud-skillcheck-cli.md` DoD — Release 发布后验证三平台 artifact + 干净容器
