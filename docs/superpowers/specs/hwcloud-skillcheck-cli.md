# Spec: hwcloud-skillcheck — 跨平台单二进制 Skill 校验 CLI
...
当前 hcloud-skills 仓库的质量校验体系已从 Python 迁移至 Go。原 Python 脚本（约 5000 行 + 测试 3650 行）已全部删除，由 `hwcloud-skillcheck` Go 二进制替代。
它面向外部用户：**从 Git 下载即用的单文件二进制**，无需 Python 解释器 / 构建环境，
即可校验自己的 hcloud-skill 仓库。

**目标产品**：`hwcloud-skillcheck` —— 一个用 Go 编写、GitHub Action 编译多平台制品、对外分发的单二进制 CLI。
外部用户下载后放入 PATH，执行 `hwcloud-skillcheck validate --root ./my-skills` 即可完成校验。

### 1.1 验收成功标准 (Definition of Done)
- [ ] 提供 `hwcloud-skillcheck` 单二进制，支持 `darwin-arm64` / `linux-amd64` / `windows-amd64` 三平台。
...
- [ ] `hwcloud-skillcheck validate --root <外部仓库>` 能正确校验一个不含本仓库硬编码结构的独立 skill 仓库。
...
外部用户仓库无对应内容。**其中 6 个已规划迁移到 hwcloud-skillcheck**（见 B 类迁移 spec），其余保留 Python：
...
```
hwcloud-skillcheck/                      # 新 Go module (module path 待 Plan 阶段按实际 repo 确定, 如 github.com/<owner>/hcloud-skills/hwcloud-skillcheck)
├── go.mod
...
- GitHub Action（`build-release.yml`）：tag push 触发，matrix `GOOS/GOARCH` 编译，
  产物 `hwcloud-skillcheck-<os>-<arch>` 上传至 GitHub Release。
...
- **默认入口**：`hwcloud-skillcheck validate --root <dir>` 为默认总入口，依次跑全部 A 类检查并汇总。
...
1. **等价性测试**：对同一 fixtures 输入，Python 脚本 vs `hwcloud-skillcheck` 的**退出码与失败项集合**一致（路径/时间戳差异归一化后比对，不要求逐字节）。
2. **零依赖测试**：干净容器（无 python3）中运行 `hwcloud-skillcheck`，全部子命令可用。
3. **跨平台测试**：三平台二进制在对应 CI runner 上 smoke 跑通。
4. **外部仓库测试**：用 `hwcloud-skillcheck validate --root <独立 skill 仓库>` 校验非本仓库结构，正确通过。
5. **自校验测试**：`hwcloud-skillcheck scan secret <x> --self-check` 对 embed fixture 扫描，无凭据泄露报告。
