# Spec: hwcloud-skillcheck B 类迁移 — GCL 契约校验 + 运行时 GCL 编排

> Status: ✅ COMPLETE — Phase 4 cleanup done (2026-07-18)
> Last updated: 2026-07-18
> Author: orchestrator (per user request)
> Parent: `docs/superpowers/specs/hwcloud-skillcheck-cli.md`

## 1. 背景与目标 (Why)

当前 hcloud-skills 仓库仍保留 8 个 Python 脚本（~1,940 行）未迁移，另有 2 个基础设施脚本。目标是**彻底消除 Python 运行时依赖**，将所有功能统一到 `hwcloud-skillcheck` Go 二进制中：

| 类别 | 数量 | 行数 | 操作 |
|------|------|------|------|
| B 类静态校验 | 6 | 1,118 | 迁移到 hwcloud-skillcheck `validate` / `check` 子命令 |
| GCL 运行时 | 2 | 822 | 迁移到 hwcloud-skillcheck `gcl` 子命令命名空间 |
| Python 基础设施 | 2 | 260 | 1 个删除（py310），1 个 Go 重写（install_hook） |

### 1.1 验收成功标准 (Definition of Done)

- [ ] 8 个 Python 脚本全部迁移到 hwcloud-skillcheck，单二进制覆盖所有功能
...
| `scripts/fixtures/` | — | fixture 已通过 hwcloud-skillcheck embed 内嵌进二进制 |
...
```
hwcloud-skillcheck gcl alarm-wire plan --summary path/to/summary.json [--write-plan]
hwcloud-skillcheck gcl alarm-wire apply --summary path/to/summary.json [--dry-run]
```
...
用根目录下的 Go 程序 `scripts/install_hook.go` 替代 `python3 scripts/install_git_hook.py`（注意：`scripts/install_hook.go` 是独立 Go 工具，不是 hwcloud-skillcheck 二进制的一部分）。

**pre-commit hook 触发条件更新**：从检测 `scripts/*.py` 改为检测 `hwcloud-skillcheck/**/*.go` + `hwcloud-skillcheck/testdata/*.py` + `scripts/*.go`。
