...

{
  ...
  "result": { "status": "success|failure|aborted",
              "resource_id": "...",
              "post_state": { ... } },
  "safety_block": null | "missing_confirmation" | "only_budget_for_cost_center"
                 | "threshold_below_spend" | "zero_threshold"
                 | "refund_without_impact_analysis" | "credential_leak"
                 | "silent_failure",
  "iter": <int>
}
...
2. Redact financial PII — replace actual monetary amounts below PII threshold (optional)
   or when the user has not consented to cost data logging.
