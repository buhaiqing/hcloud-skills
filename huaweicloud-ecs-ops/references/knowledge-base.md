...
> This file focuses on ECS-specific diagnosis and remediation.
...
| 自动修复触发 | `cpu_util > 98%` AND `duration > 5min` → CloudShell: `top -b -n 1` → kill STOP 可疑进程 |
...
| 自动修复触发 | `diskUsage > 95%` → CloudShell: `rm -rf /var/log/*.old` + `logrotate -f /etc/logrotate.conf` |
...
| 自动修复触发 | `mem_slope > 1.0%/min` AND `forecast(48h) > 95%` → Alert + auto heap dump |
...
| 自动修复触发 | `elb_health_check_fail` AND `ecs_sg_modified` → Auto restore SG rules |
